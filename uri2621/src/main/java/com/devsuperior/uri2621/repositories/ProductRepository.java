...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT products.name " +
            ...
            "AND providers.name LIKE CONCAT(:term, '%')")
    List<ProductMinProjection> searchWithSQL(Integer min, Integer max, String term);

    @Query("SELECT new com.devsuperior.uri2621.dto.ProductMinDTO(obj.name) " +
            "FROM Product obj " +
            "WHERE obj.amount BETWEEN :min AND :max " +
            "AND obj.provider.name LIKE CONCAT(:term, '%')")
    List<ProductMinDTO> searchWithJPQL(Integer min, Integer max, String term);
}
